            ...
            const parseRecursive = (t, rootIndex) => {
                ...
                const breakToken = () => {
                    ...
                    const semanticTokens = nodes.filter(n => {
                        return n.type != 'linebreak' && n.type != 'inlinecomment' && n.type != 'comment';
                    });
                    if (
                    ...
                    if (subnode.type === 'name') {
                        ...
                        for (let j = i; j < nodes.length; j++) {
                            if (nodes[j].type === 'semicolon' || nodes[j].type === 'block') {
                                ...
                                if (j + 1 < nodes.length && nodes[j + 1].type === 'inlinecomment') {
                                    skipTo = j + 2;
                                }
                                break;
                            }
        ...
        print(path, options, print) {
            ...
            const preparseRecursive = node => {
                ...
                for (let i = 0; i < node.content.length; i++) {
                    ...
                    for (let j = i - 1; j >= 0; j--) {
                        if (node.content[j].type != 'comment' && node.content[j].type != 'inlinecomment') {
                            let pos;
                            if (j > 0) {
            ...
            const getNameColEnd = (node, all, longest = 0, indents = 0) => {
                ...
                if (
                    ...
                    node.content.forEach(n => {
                        ...
                        longest = Math.max(
                            getNameColEnd(n, all, longest, node.type === 'block' ? indents + 1 : indents),
                            longest,
                        );
                    });
                }
                ...
                for (let i = 0; i < node.content.length; i++) {
                    ...
                    if (subnode.type === 'name') {
                        ...
                        if (
                            ...
                            getLineLength(directiveDocs) + subnode.content.length + 1 > options.printWidth &&
                            i >= 1 &&
                        ...
                        directiveDocs.push(indents);
                        directiveDocs.push('}');
                        if (node.content.length > i + 1 && node.content[i + 1].type != 'inlinecomment') {
                            directiveDocs.push('\n');
                        }
                    }
                    if (
                        ...
                        (directiveDocs.length > 0
                            ? [' ', '\t', undefined].indexOf(directiveDocs.at(-1)?.at(-1)) === -1
                            : true)
                    ) {
                        directiveDocs.push(' ');
                    }
